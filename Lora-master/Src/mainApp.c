...
void mainApp()
{
	...
	*/
	key1_count = 0;
	u32 timTx = 0, timTxdone = 0, timeRxdone = 0, timRx = 0;
	/*
	...
	{		
		switch(key1_count)
		{
			...
			case 0://lora master Tx
				{
					u8 Tx_Packet[20];
					sprintf((char*)sx1276_7_8Data,"2_hahuhihoheha%d\n", i++);	
					HAL_Delay(800);
					timer_measure_start();
					sx1276_7_8_LoRaEntryTx();
					timTx = timer_measure_stop();
					
					timer_measure_start();
					sx1276_7_8_LoRaTxPacket();	
					timTxdone = timer_measure_stop();
					
					sprintf((char*)Tx_Packet, "TimeTx: %d, TimeTxdone: %d, TimeRx: %d, Data sent: %s\n",timTx,timTxdone,timRx, (char*)sx1276_7_8Data);
					printUSB(Tx_Packet);	
					
					timer_measure_start();
					sx1276_7_8_LoRaEntryRx();
					timRx = timer_measure_stop();
